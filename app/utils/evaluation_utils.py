import numpy as np
def evaluation_metrics_helper(scores, labels, cats, window, threshold_percentile):
    ...
    cat_dict = {}
    normal_mask = cats == 0
    threshold = np.percentile(current_score, threshold_percentile)
    ...
    for cat in np.unique(cats):
        ...
        cat_mask = cats == cat
        recall = np.sum(
            current_score[cat_mask] > threshold
        ) / np.sum(cat_mask) if np.sum(cat_mask) > 0 else 0.0
    ...
    recall = TP / np.sum(labels == 1) if np.sum(labels == 1) > 0 else 0.0
