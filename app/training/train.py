...
def fit(
    ...
    name (str): experiment name (logging)
    ...
    warmup_steps = int((len(train_loader) * num_epochs) * 0.07) # 7% of total steps
    scheduler = WarmupCosineScheduler(
        optimizer, 
        warmup_steps = warmup_steps,
        total_steps = total_steps,
    ...
    for epoch in range(num_epochs):
        if epoch == int(num_epochs * stop_epoch_ratio): break
        ...
        with torch.no_grad():
            ...
            if (epoch+1) % 1 == 0:
                print(f"|{name}| train = {avg_train_loss:.4f} | test= {avg_eval_loss:.4f} | LR: {scheduler.get_lr():.2e}")
            ...
            if earlyStopper(avg_eval_loss):
                print(f"| experiment: {name} | epoch {epoch + 1}, train: MSE {avg_train_loss:.4f}, test MSE: {avg_eval_loss:.4f}")
