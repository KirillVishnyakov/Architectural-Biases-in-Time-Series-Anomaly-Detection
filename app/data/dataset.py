...
class forecasting_Dataset(data.Dataset):
    ...
    def __init__(self, device, cfg, window_size, horizon = 1, scaler = None, start = 0, end = 90000, train = True):
        ...
        if self.train:
            self.dataset = pd.read_csv(cfg.data_dir / "data.csv", skiprows=range(1, start+1), nrows=end - start).drop(["y", "category", "timestamp"], axis = 1)
            self.scaler = RobustScaler().fit(self.dataset)
            
        else:
            self.dataset = pd.read_csv(cfg.data_dir / "data.csv", skiprows=range(1, start+1), nrows=end - start).drop(["timestamp"], axis = 1)
...
class AE_Dataset(data.Dataset):
    ...
    def __init__(self, cfg, device, lookback_window, scaler = None, start = 0, end = 90000, train = True):
        self.device = device
